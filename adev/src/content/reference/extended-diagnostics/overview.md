...
O compilador Angular inclui "diagnósticos estendidos" que identificam muitos desses padrões, para avisar desenvolvedores sobre possíveis problemas e reforçar boas práticas comuns dentro de uma base de código.
...
| :---------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `warning`         | Padrão - O compilador emite o diagnóstico como um aviso, mas não bloqueia a compilação. O compilador ainda sairá com código de status 0, mesmo que avisos sejam emitidos.      |
...
<docs-code language="json">
{
  "angularCompilerOptions": {
    "extendedDiagnostics": {
      ...
      "checks": {
        // Mapeia o nome da verificação para sua categoria.
...
Isso é necessário para permitir que o compilador entenda melhor os tipos de template Angular e forneça diagnósticos precisos e significativos.
...
No entanto, definir `"defaultCategory": "error"` promoverá tais avisos a erros rígidos.
Isso pode fazer com que uma atualização de versão minor introduza erros de compilação, o que pode ser visto como uma breaking change não conforme com semver.
Quaisquer novos diagnósticos podem ser suprimidos ou rebaixados para avisos via a [configuração](#configuration) acima, então o impacto de um novo diagnóstico deve ser mínimo para
projetos que tratam diagnósticos estendidos como erros por padrão.
Ter erro como padrão é uma ferramenta muito poderosa; apenas esteja ciente dessa ressalva de semver ao decidir se `error` é o padrão certo para seu projeto.

## Novos Diagnósticos

O time do Angular está sempre aberto a sugestões sobre novos diagnósticos que podem ser adicionados.
Diagnósticos estendidos geralmente devem:

- Detectar um erro comum e não óbvio de desenvolvedor com templates Angular
- Articular claramente por que este padrão pode levar a bugs ou comportamento não intencional
- Sugerir uma ou mais soluções claras
- Ter uma taxa baixa, preferencialmente zero, de falsos positivos
- Aplicar-se à vasta maioria de aplicações Angular (não específico para uma biblioteca não oficial)
- Melhorar a correção ou performance do programa (não estilo, essa responsabilidade cai em um linter)
