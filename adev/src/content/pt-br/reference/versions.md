...
As tabelas a seguir descrevem as versões do Node.js, TypeScript e RxJS que cada versão do
Angular requer.
...
## Versões do Angular sem suporte

Esta tabela cobre versões do Angular que não estão mais sob suporte de longo prazo (LTS). Esta
informação estava correta quando cada versão saiu do LTS e é fornecida sem nenhuma garantia adicional.
...
Até o Angular v9, as versões do Angular e do Angular CLI não eram sincronizadas.
...
O Angular usa o [Baseline "amplamente disponível"](https://web.dev/baseline) para definir o
suporte a navegadores. Para cada versão principal, o Angular suporta navegadores incluídos no Baseline de uma
data escolhida próxima à data de lançamento dessa versão principal.

O Baseline "amplamente disponível" inclui navegadores lançados há menos de 30 meses (2,5 anos)
da data escolhida dentro do conjunto de navegadores principais do Baseline (Chrome, Edge, Firefox, Safari) e
tem como alvo suportar aproximadamente 95% dos usuários da web.

| Angular | Data do Baseline | Conjunto de Navegadores                 |
| ------- | ---------------- | --------------------------------------- |
...
Versões do Angular anteriores à v20 suportam as seguintes versões específicas de navegadores:

| Navegador | Versões suportadas                              |
| :-------- | :---------------------------------------------- |
| Chrome    | 2 versões mais recentes                         |
| Firefox   | mais recente e extended support release \(ESR\) |
...
O Angular é construído sobre os padrões mais recentes da plataforma web.
Ter como alvo uma gama tão ampla de navegadores é desafiador porque eles não suportam todos os recursos dos navegadores modernos.
Você compensa isso carregando scripts polyfill \("polyfills"\) para os navegadores que você precisa suportar.
Veja instruções sobre como incluir polyfills no seu projeto abaixo.
...
ÚTIL: Polyfills não podem magicamente transformar um navegador antigo e lento em um moderno e rápido.

## Habilitando polyfills com projetos CLI

O [Angular CLI](tools/cli) fornece suporte para polyfills.
Se você não está usando o CLI para criar seus projetos, veja [Instruções de polyfill para usuários que não usam CLI](#polyfills-for-non-cli-users).
...
## Polyfills para usuários que não usam CLI {#polyfills-for-non-cli-users}

Se você não está usando o CLI, adicione seus scripts polyfill diretamente à página web host \(`index.html`\).
...
<script>
  /**
   * você pode configurar algumas flags de zone que podem desabilitar a interceptação de zone para algumas
   * atividades assíncronas para melhorar o desempenho de inicialização - use essas opções apenas
   * se você souber o que está fazendo, pois isso pode resultar em bugs difíceis de rastrear.
   */
  // __Zone_disable_requestAnimationFrame = true; // desabilita patch requestAnimationFrame
  // __Zone_disable_on_property = true; // desabilita patch onProperty como onclick
  // __zone_symbol__UNPATCHED_EVENTS = ['scroll', 'mousemove']; // desabilita patch de eventNames especificados
  /*
   * nas ferramentas de desenvolvedor do Edge, o addEventListener também será envolvido por zone.js
   * com a seguinte flag, ele ignorará o patch `zone.js` para o Edge.
   */
  // __Zone_enable_cross_context_check = true;
</script>
<!-- zone.js requerido pelo Angular -->
