...
Para adicionar o pacote `@angular/localize`, use o seguinte comando para atualizar os arquivos `package.json` e de configuração do TypeScript no seu projeto.
...
Também adiciona a linha `/// <reference types="@angular/localize" />` no topo do arquivo `main.ts`, que é a referência para a definição de tipo.
...
Se `@angular/localize` não estiver instalado e você tentar construir uma versão localizada do seu projeto (por exemplo, ao usar os atributos `i18n` em templates), o [Angular CLI][CliMain] gerará um erro, que conterá as etapas que você pode seguir para habilitar i18n para seu projeto.
...
| `--use-at-runtime` | Se definido, então `$localize` pode ser usado em tempo de execução. Além disso, `@angular/localize` é incluído na seção `dependencies` do `package.json`, em vez de `devDependencies`, que é o padrão. | `boolean`     | `false`      |
...
<docs-pill-row>
  <docs-pill href="guide/i18n/locale-id" title="Referir-se a locales por ID"/>
</docs-pill-row>
